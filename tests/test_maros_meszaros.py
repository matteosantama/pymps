...
def _parse_reference() -> list[tuple[str, int, float]]:
    rows = []
    for line in REFERENCE.strip().splitlines():
        parts = line.split()
        name, _m, n, _nz, _qn, _qnz, opt = parts
        rows.append((name, int(n), float(opt)))
    return rows


PROBLEMS = _parse_reference()


@pytest.mark.parametrize("name,n,opt", PROBLEMS, ids=[r[0] for r in PROBLEMS])
def test_maros_meszaros(name: str, n: int, opt: float, problem_paths: dict[str, str]):
    problem_path = problem_paths[name]
    qp = pymps.read_f64(problem_path)

    assert qp.c.shape == (n,)
    assert qp.p.shape == (n, n)
    assert qp.a.shape[1] == n
    assert np.all(np.isfinite(qp.c))
    ...
    p_upper = sp.sparse.triu(qp.p, format="csc")
    solver = clarabel.DefaultSolver(p_upper, qp.c, qp.a, qp.b, cones, settings)
    solution = solver.solve()

    assert solution.status == clarabel.SolverStatus.Solved
    solved_opt = solution.obj_val + qp.r
    assert solved_opt == pytest.approx(opt, rel=1e-3, abs=1e-4)
